...
type TabType = 'active' | 'top16' | 'organic';
...
const TrendingScreen = () => {
  ...
  // For now, showing all posts in each tab
  // You can implement different filtering logic based on your backend
  ...
  const renderHeader = () => (
    <View>
      ...
      <View className='mx-6 mt-4'>
        <View
          className={`rounded-2xl p-4 flex-row items-center justify-between ${
            isEligible ? 'bg-green-500/20 border border-green-500/50' : 'bg-red-500/20 border border-red-500/50'
          }`}
        >
      ...
      {/* Tabs */}
      <View className='flex-row justify-between items-center gap-3 mt-6 mx-6'>
        {(['active', 'top16', 'organic'] as TabType[]).map((tab) => (
          <TouchableOpacity
            key={tab}
          ...
          >
            ...
            <Text className='text-primary font-roboto-semibold text-sm'>
              {tab === 'active' ? 'Active' : tab === 'top16' ? 'Top 16' : 'Organic'}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
