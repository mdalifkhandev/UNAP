...
const MediaPickers: React.FC<MediaPickersProps> = ({
  ...
  return (
    <View className='flex-row justify-between px-6 mt-6'>
      <TouchableOpacity
        onPress={onPickPhoto}
        className='bg-[#FFFFFF0D] px-5 py-4 rounded-lg items-center flex-1 mr-3'
        activeOpacity={0.7}
      >
        <Feather name='image' size={32} color='#00E6E6' />
        <Text className='text-white font-roboto-regular mt-2 text-xs'>
          Photo
        </Text>
      </TouchableOpacity>
      ...
      <TouchableOpacity
        onPress={onPickAudio}
        className='bg-[#FFFFFF0D] px-5 py-4 rounded-lg items-center flex-1'
        activeOpacity={0.7}
      >
